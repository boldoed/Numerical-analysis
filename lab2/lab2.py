...
if __name__ == "__main__":
    try:
        ...
        solution_opt, matr = optimal_exclusion([row.copy() for row in matrix])
        residual_opt = calculate_residual(matrix, solution_opt)
        
        # Решение методом вращений
        solution_rot = rotation_method([row.copy() for row in matrix])
        residual_rot = calculate_residual(matrix, solution_rot)
